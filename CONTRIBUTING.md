...
The release workflow also enforces the fail-closed npm publication policy in `publisher.json`. External publication must be explicitly allowed by the founder-owner, that authorization must be effective while LLC formation is pending, and the publisher identity, policy URLs, namespace, repository provenance, package identity, and registry metadata must match exactly. Generic provider authentication and review requirements do not block the npm artifact; they remain separate gates for MCP Registry and provider submissions.
